...
function saveAddresses(deployments) {
  ...
  fs.writeFileSync(contractsDir + "/contract.ts", `export default { ${payload} }`);
}
...
function saveAbi(deployments) {
  ...
  deployments.map((contract) => {
    let artifact;
    if (contract.name == "MockLP1" || contract.name == "MockLP2" || contract.name == "MockLP3") {
      artifact = artifacts.readArtifactSync("MockLP");
    } else {
      artifact = artifacts.readArtifactSync(contract.name);
    }

    fs.writeFileSync(contractsDir + `/${contract.name}.json`, JSON.stringify(artifact.abi, null, 2));
  });
}
...
function saveBytecode(deployments) {
  ...
  deployments.map((contract) => {
    ...
    fs.writeFileSync(contractsDir + `/${contract.name}.json`, JSON.stringify(artifact.bytecode, null, 2));
  });
}
