...
function saveFrontendFiles(deployments) {
  ...
  fs.writeFileSync(contractsDir + "/abi/Kalomira.json", JSON.stringify(KalomiraArtifact.abi, null, 2));
  fs.writeFileSync(contractsDir + "/bytecode/Kalomira.json", JSON.stringify(KalomiraArtifact.bytecode, null, 2));

  /* Deploy Kalomira abi and bytecode */
  const ibKAIArtifact = artifacts.readArtifactSync("ibKAI");
  fs.writeFileSync(contractsDir + "/abi/ibKAI.json", JSON.stringify(ibKAIArtifact.abi, null, 2));
  fs.writeFileSync(contractsDir + "/bytecode/ibKAI.json", JSON.stringify(ibKAIArtifact.bytecode, null, 2));

  /* Deploy Kalomira abi and bytecode */
  const MockLPArtifact = artifacts.readArtifactSync("MockLP");
  fs.writeFileSync(contractsDir + "/abi/MockLP.json", JSON.stringify(MockLPArtifact.abi, null, 2));

  fs.writeFileSync(contractsDir + "/bytecode/MockLP.json", JSON.stringify(MockLPArtifact.bytecode, null, 2));

  /* Deploy Kalomira abi and bytecode */
  const MasterChefArtifact = artifacts.readArtifactSync("MasterChef");
  fs.writeFileSync(contractsDir + "/abi/MasterChef.json", JSON.stringify(MasterChefArtifact.abi, null, 2));
  fs.writeFileSync(contractsDir + "/bytecode/MasterChef.json", JSON.stringify(MasterChefArtifact.bytecode, null, 2));
}
