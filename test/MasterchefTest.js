...
describe("Testing", function(){
    ...
    beforeEach(async function(){
        ...
        lp1 = await LP1.deploy("LPToken 1", "LP1", TRANSFER_AMT * 5);
        await lp1.deployed();

        const LP2 = await ethers.getContractFactory("MockLP");
        lp2 = await LP2.deploy("LPToken 2", "LP2", TRANSFER_AMT * 5);
        await lp2.deployed();
        // 100 KALO per block starting at block 100 with bonus until block 1000
        const MasterChef = await ethers.getContractFactory("MasterChef");
        masterchef = await MasterChef.deploy(kaloToken.address, owner.address, 100, 100, 1000);
        await masterchef.deployed();

        await kaloToken._transferOwnership(masterchef.address);

        await Promise.all([
            lp1.connect(owner).transfer(alice.address, TRANSFER_AMT),
            lp2.connect(owner).transfer(alice.address, TRANSFER_AMT),

            lp1.connect(owner).transfer(bob.address, TRANSFER_AMT),
            lp2.connect(owner).transfer(bob.address, TRANSFER_AMT),

            lp1.connect(owner).transfer(carol.address, TRANSFER_AMT),
            lp2.connect(owner).transfer(carol.address, TRANSFER_AMT),
        ])
    });
