...
contract FeeProvider is IFeeProvider, FeeProviderState, WithAddressProvider, OwnableUpgradeable {
  ...
  function chargeFee(uint256 amount, bool isMintOrRedeem) external override returns (uint256, uint256) {
    if (!_hasApproved) {
      // approve treasury
    ...
    (uint256 leftAmount, uint256 feeAmount) = calcFee(amount, isMintOrRedeem);
  ...
  function calcFee(uint256 amount, bool isMintOrRedeem) public view override returns (uint256, uint256) {
    uint256 feeAmount = amount.div(10000).mul(isMintOrRedeem ? _mintFee : _redeemFee);
  ...
  function _getMintFeeAmount(uint256 amount) internal {}
}
