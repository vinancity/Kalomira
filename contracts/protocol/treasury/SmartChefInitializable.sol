...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
  ...
  // Whether we mint rewardToken or transfer from source
  bool public mintRewards;
  ...
  /**
   * @dev User transfers staking tokens to Treasury first, then we pull the tokens from there
   * @notice Deposit staked tokens and collect reward tokens (if any)
   * @param _amount: amount to withdraw (in stakedToken)
   */
  function deposit(address account, uint256 _amount) external nonReentrant onlyOwner {
  ...
  /**
   * @dev rewards and staked tokens transferred directly to account
   * @notice Withdraw staked tokens and collect reward tokens
   * @param _amount: amount to withdraw (in rewardToken)
   */
  function withdraw(address account, uint256 _amount) external nonReentrant onlyOwner {
    UserInfo storage user = userInfo[account];
   ...
   */
  function emergencyWithdraw(address account) external nonReentrant onlyOwner {
    UserInfo storage user = userInfo[account];
    ...
    if (amountToTransfer > 0) {
      stakedToken.safeTransfer(address(account), amountToTransfer);
    }
