...
export class Dapp extends React.Component {
	...
	render() {
		...
		// a loading component.
		if (
			!this.state.Native_balance ||
			!this.state.KALO_tokenData ||
			!this.state.KALO_balance ||
			!this.state.ibKAI_tokenData ||
			!this.state.ibKAI_balance ||
			!this.state.LP1_tokenData ||
			!this.state.LP1_balance ||
			!this.state.LP2_tokenData ||
			!this.state.LP2_balance ||
			!this.state.LP3_tokenData ||
			!this.state.LP3_balance ||
						...
						<div className="container p-4">
							<div className="row">
								<div className="col-12">
									<h1>
										{this.state.KALO_tokenData.name} (
										{this.state.KALO_tokenData.symbol})
									</h1>
									<p>
									...
									<p>
										You have{" "}
										<b>
											{(this.state.ibKAI_balance / 10 ** 18).toFixed(4)}{" "}
											{this.state.ibKAI_tokenData.symbol}
										</b>
										{", "}
										<b>
											{(this.state.LP1_balance / 10 ** 18).toFixed(4)}{" "}
											{this.state.LP1_tokenData.name}
										</b>
										{", "}
										<b>
											{(this.state.LP2_balance / 10 ** 18).toFixed(4)}{" "}
											{this.state.LP2_tokenData.name}
										</b>
										{", "}and{" "}
										<b>
											{(this.state.LP3_balance / 10 ** 18).toFixed(4)}{" "}
											{this.state.LP3_tokenData.name}
										</b>
										.
									</p>
									<div className="row">
										<div className="col-md-auto">
											Total LP Staked:
											<h2>
												{this.state.LP_staked
													? (this.state.LP_staked / 10 ** 18).toFixed(4)
													: "N/A"}
												{" LP"}
											</h2>
										</div>
										<div className="col-md-auto">
											...
											<h2>
												{this.state.KALO_harvest
													? (this.state.KALO_harvest / 10 ** 18).toString()
													: "N/A"}{" "}
												{this.state.KALO_tokenData.symbol}
											</h2>
										</div>
										<div className="col-md-auto">
											...
											<h2>
												{(this.state.KALO_balance / 10 ** 18).toString()}{" "}
												{this.state.KALO_tokenData.symbol}
											</h2>
										</div>
									</div>
	...
	async _getTokenData() {
		...
		this.setState({
			LP_tokenData: [...this.state.LP_tokenData, { name, symbol }],
		});
		...
		for (let x = 0; x < length; x++) {
			...
			UserInfo.push(
				await this._masterchef.userInfo(x, this.state.selectedAddress)
			);
			PendingRewards.push(
				await this._masterchef.pendingKalo(x, this.state.selectedAddress)
			);
		}
	...
	async _updateBalance() {
		const Native_balance = await this._provider.getBalance(
			this.state.selectedAddress
		);
		this.setState({ Native_balance });

		const KALO_balance = await this._kalToken.balanceOf(
			this.state.selectedAddress
		);
		this.setState({ KALO_balance });

		const KALO_harvest = await this.totalHarvestable(
			this.state.selectedAddress
		);
		this.setState({ KALO_harvest });

		const ibKAI_balance = await this._ibKaiToken.balanceOf(
			this.state.selectedAddress
		);
	...
	async _checkNetwork() {
		...
		this.setState({
			networkError: `Please connect Wallet to ${
				this._wallet === window.ethereum ? ethNetwork : kaiNetwork
			}`,
		});
