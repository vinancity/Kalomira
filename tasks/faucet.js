...
task("faucet", "Sends ETH and tokens to an address")
	...
	.setAction(async ({ receiver }) => {
		...
		const addressesFile =
			__dirname + "/../frontend/src/config/constants/contracts.json";
    ...
    await tx.wait();*/
		...
		const tx1 = await lp1
			.connect(sender)
			.transfer(receiver, getBigNumber(numTokens));
		await tx1.wait();
		...
		console.log(
			`Transferred 1 ETH,  ${numTokens} MockLP1, ${numTokens} MockLP2, ${numTokens} MockLP3 to ${receiver}`
		);
	});
